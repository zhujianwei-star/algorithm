...
/**
 * @ClassName : StrStr
 * @Description: 实现String.index()方法
 * @Author : Zhujw
 * @CreateDate : 2022/2/23 15:42
 */
public class StrStr {
    ...
    public static void main(String[] args) {
        String s = "hello";
        String s1 = "lo";
        System.out.println(solution(s, s1));
    }
    ...
    /*private static int solution(String haystack, String needle) {
        if (Objects.equals(needle, "") || Objects.equals(haystack, ""))
            return 0;
        int i = 0, j = 0;
        while (i < haystack.length() && j < needle.length()) {
            if (haystack.charAt(i) == needle.charAt(j)) {
    ...
    private static int KMPSolution(String haystack, String needle) {
        if (needle.length() == 0)
            return 0;
        int i = 0;
        int j = 0;
